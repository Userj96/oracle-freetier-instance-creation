...
- Gmail notification
- A persistent VM SSH key pair is created locally once and the same public key is supplied on every launch attempt
...
5. Edit the **`oci.env`** file and fill in the necessary details. Refer [below for more information](https://github.com/mohankumarpaluru/oracle-freetier-instance-creation#environment-variables) `oci.env` fields.
    ...
    The setup script also creates a persistent VM SSH key pair (`id_rsa` and `id_rsa.pub`) in the project directory if one does not already exist. Keep `id_rsa` private. The public key is reused for every launch attempt.
...
- `SSH_AUTHORIZED_KEYS_FILE`: Absolute path to the persistent VM SSH public key used for the instance.
- `SSH_AUTHORIZED_KEYS`: Optional raw SSH public key. This is the preferred form for GitHub Actions. It should contain the contents of your local `id_rsa.pub`/`id_ed25519.pub`; the private key is never sent to OCI as instance metadata and should remain on your machine.
- `IMAGE_NAME_FILTER`: Case-insensitive substring filter for the OCI image display name. For the default A1 ARM configuration this is `Minimal-aarch64`, which distinguishes the ARM Minimal image from the separate x86 Minimal image.
...
This repository can run the capacity-hunting loop on a disposable GitHub-hosted runner, so the local laptop or OCI Micro instance does not need to stay online while hunting.

### Manual run

1. Add these repository secrets under **Settings → Secrets and variables → Actions**:
   - `OCI_CONFIG` — the complete OCI API config file contents.
   - `OCI_API_PRIVATE_KEY` — the complete OCI API private key contents.
   - `OCI_ENV` — the complete `oci.env` contents for the account/region you want to use.
2. Open **Actions → OCI Capacity Hunter**.
3. Choose **Run workflow**.

The workflow reconstructs the credential files only on the ephemeral runner. Private credentials are not uploaded as artifacts.

### Scheduled hunting

The workflow is scheduled for `17 * * * *` (17 minutes past every hour), rather than exactly on the hour. GitHub-hosted scheduled workflows can still be delayed by platform load, so the minute value is not a guarantee of exact start time.

Each run may hunt for up to 59 minutes. The workflow uses concurrency protection so a delayed scheduled run does not overlap another hunter run.

The workflow runs `python main.py` directly so the Python process remains attached to the Actions job and its progress/errors are visible in the job log.

### GitHub Actions artifacts

After every run, diagnostic artifacts may include `launch_instance.log`, `setup_and_info.log`, `images_list.json`, `INSTANCE_CREATED`, and error logs when present. OCI config files, private keys, `oci.env`, and other credentials are intentionally excluded.

### Image selection

When `OCI_IMAGE_ID` is empty, the script lists images for the configured shape, writes a diagnostic `images_list.json`, and filters by operating system, OS version, and `IMAGE_NAME_FILTER`. Zero matches are treated as an error; multiple matches are treated as ambiguous and require an exact `OCI_IMAGE_ID` instead of silently choosing one. The default A1 filter is `Minimal-aarch64`, matching the separately listed ARM Minimal Ubuntu image. Oracle's current image catalog explicitly lists `Canonical-Ubuntu-24.04-Minimal-aarch64-*` images. See the [Oracle Ubuntu 24.04 image catalog](https://docs.oracle.com/en-us/iaas/images/ubuntu-2404/).

### Current ARM configuration

The default ARM configuration in this fork is `VM.Standard.A1.Flex` with 2 OCPUs and 12 GB RAM, Canonical Ubuntu 24.04, a 134 GB boot volume, and 120 VPUs/GB. These values remain configurable through `oci.env` where supported by the script.

### SSH key handling

Create the VM SSH key pair **locally once**. The public key is what OCI places into the new VM's `authorized_keys`; the matching private key stays on the machine you control. The hunter reuses that same public key for every capacity attempt.

For local execution, `setup_env.sh` creates `id_rsa` and `id_rsa.pub` if they do not already exist. For GitHub Actions, copy the contents of your local `id_rsa.pub` into the `SSH_AUTHORIZED_KEYS` line inside the `OCI_ENV` repository secret. The Actions runner does not generate a VM private key, and it never needs your VM private key.

Do not confuse this VM SSH key with `oci_api_private_key.pem`: the OCI API private key authenticates the hunter to OCI, while the VM SSH private key authenticates you to the instance after it is created.
